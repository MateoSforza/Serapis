...
namespace Serapis.Controladores
{
    public class VentaController
    {
        ...
        public (string Resultado, int? VentaId) RegistrarVenta(int? clienteId, List<ItemVenta> carrito, string? receta = null)
        {
            if (!carrito.Any())
                return ("El carrito está vacío.", null);

            if(SesionActual.UsuarioLogueado == null)
                return ("Usuario no logueado.", null);
            ...
            foreach (var item in carrito)
            {
                ...
                if (producto == null) return ($"Producto con ID {item.ProductoId} no encontrado.", null);
                if (producto.Stock < item.Cantidad) return ($"Stock insuficiente para {producto.Nombre}", null);
